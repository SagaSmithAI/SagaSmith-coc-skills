"""Database runtime for persistent CoC7 domain state."""
...
class Base(DeclarativeBase):
    """Declarative base shared by all CoC7 domain models."""
...
def _default_database_path() -> Path:
    ...
    return db_dir / "coc7.db"


def default_database_url() -> str:
    """Return the configured URL or the skill-local SQLite database URL."""
    configured = os.environ.get("COC7_DATABASE_URL") or os.environ.get("DATABASE_URL")
    if configured:
        return configured
...
class Database:
    """Own a CoC7 database engine and its transactional session factory."""
    ...
    def drop_schema(self) -> None:
        """Drop all CoC7 tables. Intended for isolated tests only."""
        from ..db import models  # noqa: F401
